...
人間の目が感じることのできる電磁波の波長範囲は **400nm ～ 800nm** であり、この範囲の光を**可視光**という。

| 種類 | 波長範囲 | 特徴 |
|------|---------|------|
| **紫外線（UV）** | 400nm 未満 | 人間の目には見えない。殺菌作用・蛍光作用がある |
| **可視光** | 400nm ～ 800nm | 人間が色として知覚できる光 |
| **赤外線（IR）** | 800nm 超 | 人間の目には見えない。熱として感じられ、夜間撮影などに利用される |

#### 原刺激と混色

**原刺激**とは、互いに独立した3つの色の光であり、赤（R）・緑（G）・青（B）を指す。これら3原色の光を適切な割合で混ぜ合わせることで、白を含むあらゆる色の光を再現できる。

**混色**とは、複数の色の光を混ぜ合わせることで別の色を作ることである。

#### 加法混色

**加法混色**は、赤（R）・緑（G）・青（B）の光を重ね合わせる方法である。混合するほど明るくなり、3色すべてを合わせると**白**になる。

$$
R + G = \text{Yellow},\quad G + B = \text{Cyan},\quad R + B = \text{Magenta},\quad R + G + B = \text{White}
$$

ディスプレイ・プロジェクター・スマートフォン画面など、**光の発光を利用した表示系**で使われる。

#### 減法混色

**減法混色**は、シアン（C）・マゼンタ（M）・イエロー（Y）のフィルタ（インク）を重ね合わせる方法である。混合するほど暗くなり、3色すべてを重ねると**黒**に近づく。

$$
C + M = \text{Blue},\quad M + Y = \text{Red},\quad C + Y = \text{Green},\quad C + M + Y \approx \text{Black}
$$

カラー印刷（CMYK）など、**光を吸収・反射する媒体**での発色に利用される。
...
撮像素子の前に赤・緑・青のフィルタを順番に装着して撮像し、それぞれのチャンネル画像を合成することでカラー画像が得られる。

各チャンネルの出力値は以下の積分式で表される：

$$
I_r = k \int_{\lambda} L(\lambda) \cdot F_r(\lambda) \cdot D(\lambda) \, d\lambda
$$

$$
I_g = k \int_{\lambda} L(\lambda) \cdot F_g(\lambda) \cdot D(\lambda) \, d\lambda
$$

$$
I_b = k \int_{\lambda} L(\lambda) \cdot F_b(\lambda) \cdot D(\lambda) \, d\lambda
$$

| 記号 | 意味 |
|------|------|
| $k$ | スケールのための定数 |
| $\lambda$ | 波長（積分は撮像素子が感度を持つ波長範囲にわたる） |
| $L(\lambda)$ | カメラへの入射光の分光強度分布 |
| $F_r(\lambda),\, F_g(\lambda),\, F_b(\lambda)$ | 赤・緑・青フィルタの分光透過率 |
| $D(\lambda)$ | 撮像素子の分光感度 |

#### 分光感度特性の例

産業用カラーカメラ Basler acA1300-60gc の分光感度特性（模式図）を以下に示す。R・G・Bそれぞれのチャンネルが感度ピークを持つ波長が異なることがわかる。

<p align="center"><img src="./diagrams/spectral_sensitivity.svg" width="600"></p>

---

### 2-5-3　カラー画像の撮影

カラー画像を撮影するカメラは、撮像素子の数によって**3板式**と**単板式**に分類される。

#### 1. 3板式カメラ

**3板式カメラ**とは、プリズムによって入射光をR・G・Bの3方向に分光し、それぞれ専用の撮像素子で同時に撮像する方式である。

| 特徴 | 内容 |
|------|------|
| **長所** | 各画素でR・G・Bすべての情報を直接取得できるため、高解像度・高色精度 |
| **短所** | プリズムと3枚の撮像素子が必要であり、コスト・サイズが大きい |
| **用途** | 放送用カメラ・映画撮影・精密検査など業務用途 |

#### 2. 単板式カメラ

**単板式カメラ**とは、1枚の撮像素子の各画素前面に色フィルタ（**カラーフィルタアレイ、CFA**）を貼り付ける方式である。最も広く普及しているCFAが**ベイヤー配列（Bayer pattern）**である。

**ベイヤー配列**は、R・G・G・Bの2×2パターンを繰り返す構成をとる。緑（G）を2倍配置するのは、人間の視覚が緑波長帯に最も敏感なためである。

> **注意**：各画素では1チャンネル分の情報しか取得できないため、欠損した他の2チャンネルの値は補完（補間）によって求める必要がある。

#### カラーデモザイキング

**カラーデモザイキング（Color Demosaicing）**とは、ベイヤー配列で撮影したモザイク状の画像から、全画素にわたるRGB3チャンネルのカラー画像を再構成する処理である。

<p align="center"><img src="./diagrams/color_demosaicing.svg" width="680"></p>

各チャンネルに分離すると、色フィルタが存在しない画素位置では値が欠損する。この欠損値を周辺の同色画素から補間することで完全なRGB画像が得られる。

##### 方法1：バイリニア補間（Bilinear Interpolation）

最も単純な補間方法。注目画素の周辺に存在する**同色の画素値の平均**で欠損値を補間する。

<p align="center"><img src="./diagrams/bayer_bilinear.svg" width="560"></p>

| 特徴 | 内容 |
|------|------|
| **長所** | 計算量が少なく、実装が簡単 |
| **短所** | エッジ付近で**偽色（カラーアーティファクト）**が発生しやすい |

##### 方法2：エッジセンシング補間（Edge-Sensing Interpolation）

水平・垂直方向の輝度勾配を計算し、**エッジを横切らない方向**の画素値で補間する方法。

<p align="center"><img src="./diagrams/edge_sensing.svg" width="560"></p>

| 特徴 | 内容 |
|------|------|
| **長所** | エッジ付近での偽色を大幅に低減できる |
| **短所** | バイリニア補間より計算量が多い |
